...
gb_temp$PET <- thornthwaite(gb_temp$temp, 53.958694)

plot(gb_prcp$prcp ~ gb_temp$year)

spei3 <- spei(gb_prcp$prcp - gb_temp$PET, 3)



cor.spei.mat <- cor(cbind(PCGA.out$pop[PCGA.out$period,], climate[PCGA.out$period,]), 
                    use = "pairwise.complete", 
                    method = "spearman")[1:ncol(PCGA.out$pop), 
                                         (ncol(PCGA.out$pop)+1):(ncol(PCGA.out$pop)+ncol(climate))]
